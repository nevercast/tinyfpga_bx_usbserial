...
pll pll192( .clock_in(CLK), .clock_out(clk_192mhz), .locked(clk_locked) );
...
The pins in the interface to the module (`USBP` and `USBN`) are the raw device pins, the direction control logic is internal to `usb_uart_i40`.  The original usb_uart module is retained below to facilitate reuse in other architectures.
